...
architecture rtl of tcp_server is
    ...
    signal syn_received : std_logic := '0';
            ...
            case state is
                ...
                when LISTEN =>
                    if in_valid = '1' then
                        ...
                        case rx_cnt is
									 -- Preamble (bajt 0-6, 0xAA) - preskačemo
									 when 0 to 6 => null;
									 ...
									 when 13 =>
										  ...
										  if (rx_dest_mac(47 downto 8) & in_data) /= SERVER_MAC then
												flag_error <= '1';
												report "Neispravna mac adresa servera";
										  end if;
								
									 -- Učitavanje Source MAC (Client MAC) (bajt 14-19)
									 when 14 => reg_c_mac(47 downto 40) <= in_data;
									 when 15 => reg_c_mac(39 downto 32) <= in_data;
									 when 16 => reg_c_mac(31 downto 24) <= in_data;
									 when 17 => reg_c_mac(23 downto 16) <= in_data;
									 when 18 => reg_c_mac(15 downto 8)  <= in_data;
									 when 19 => reg_c_mac(7 downto 0)   <= in_data;
								
									 -- EtherType (bajt 20-21) - preskačemo
								
									 -- IP Header
									 when 34 => reg_c_ip(31 downto 24) <= in_data;
									 when 35 => reg_c_ip(23 downto 16) <= in_data;
									 when 36 => reg_c_ip(15 downto 8)  <= in_data;
									 when 37 => reg_c_ip(7 downto 0)   <= in_data;
									 ...
									 when 41 =>
										  ...
										  if (rx_dest_ip(31 downto 8) & in_data) /= SERVER_IP then
												flag_error <= '1';
												report "Neispravna ip adresa servera";
										  end if;
								
									 -- TCP Header
									 when 42 => reg_c_port(15 downto 8) <= in_data;
									 when 43 => reg_c_port(7 downto 0)  <= in_data;
									 ...
									 when 45 =>
										  ...
										  if (rx_dest_port(15 downto 8) & in_data) /= SERVER_PORT then
												flag_error <= '1';
												report "Neispravan port servera";
										  end if;
									 ...
									 when 55 =>
										  --rx_flags <= in_data;
										  -- Provjera SYN flaga
										  if flag_error = '0' then        -- samo ako nije bilo greške
												if in_data(TCP_SYN) = '1' then
													 if syn_received = '0' then
														  flag_syn <= '1';   -- prvi SYN
														  syn_received <='1';
													 else
														  flag_error <= '1'; -- duplikovani SYN → error
													 end if;
												end if;
										  end if;
                ...
                when WAIT_ACK =>
                    if in_valid = '1' then
                        if in_sop = '1' then 
								...
								--report "RX counter = " & integer'image(rx_cnt);
                        if rx_cnt = 55 and in_data(TCP_ACK) = '1' then 
									flag_ack <= '1';	
								end if;
						  ...
						  if in_eop = '1' then
                            if flag_ack = '1' then 
										...
										report "Nije primljen ACK paket";
										state <= CLOSED; 
										tx_packet_kind <= TX_RST;
                            end if;
                     end if;
                ...
                when ESTABLISHED =>
                    -- Scenario 3: Detekcija duplog SYN-a
						  report "Konekcija uspostavljena";  -- DEBUG
                    ...
                    if in_valid = '1' then
                        ...
                        if rx_cnt = 55 and in_data(TCP_SYN) = '1' then 
									flag_syn <= '1';
								   report "Dupli SYN paket";	
								end if;
